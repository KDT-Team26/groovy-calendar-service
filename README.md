...
## 1. Repo: groovy-calendar-service
...
하지 않습니다. 캘린더 화면에는 유저 이름이 표시되지 않기 때문입니다. 
- 대신 study-service와는 스터디 멤버십 확인, "내 스터디 옵션" 조회를 위해 자체 로컬 클라이언트(`StudyServiceClient`)로
통신합니다.

### 데이터베이스

- **DB(스키마)명**: `calendar_db` (일정 서비스 전용 DB)
- **일정 서비스 계정**: `calendar_service` (다른 서비스 DB 접근 권한 없음)
...
이 레포는  MySQL, Kafka, identity-service, study-service가 먼저 떠 있어야 정상 동작합니다.
